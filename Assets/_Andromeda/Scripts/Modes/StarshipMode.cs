...
namespace Assets.Scripts.Modes
{
    public class StarshipMode : MonoBehaviour, IMode
    {
        ...
        private Planet currentPlanetToLand = null;
        ...
        public void Land(float value)
        {
            if (availableToLand)
            {
                Vector3 landPoint = FindClosestLandPoint(currentPlanetToLand);
                landingMode.Play(currentStarship, currentStarshipAttributes, currentPlanetToLand, landPoint, currentShipCamera);
                Stop();
            }
        }
        ...
        {
            float distance = Vector3.Distance(currentStarship.transform.position, currentPlanetToLand.transform.position);
            if (distance > LANDING_DISTANCE + currentPlanetToLand.Settings.radius)
            {
                availableToLand = false; ;
                currentPlanetToLand = null;
                Debug.Log("NOT LAND");
        ...
        private Vector3 FindClosestLandPoint(Planet planet)
        {
            List<Vector3> points = planet.FreePointsToLand;
